...
request({ url:url , json:true },(error,response)=> {
    if (error) {
        ...
        callback(undefined, response.body.current.weather_descriptions[0]+'. It is currently ' + response.body.current.temperature + ' degress out. But it feels like ' + response.body.current.feelslike + ' degress out. Humidity is '+ response.body.current.humidity+'%.')
    }
})
